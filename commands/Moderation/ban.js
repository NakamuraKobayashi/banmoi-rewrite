...
module.exports = {
  ...
  run: async (client, message, args) => {
    ...
    try {
      const bot = client;
      if (!args[0])
        return message.reply(`**Hãy đề cập một người dùng bạn muốn Cấm khỏi máy chủ**`).then(message => {setTimeout(() => message.delete(), 3000)
      ...
      if (!banMember)
        return message.reply(`**Người này không có trong máy chủ này**`) .then(message => {setTimeout(() => message.delete(), 3000)
      ...
      if (!banMember.bannable)
        return message.reply(`**Không thể cấm người này**`) .then(message => {setTimeout(() => message.delete(), 3000)
      ...
      try {
        ...
        banMember
          .send(
            ` Xin chào, bạn đã bị Cấm khỏi máy chủi} **${
              message.guild.name
            }** Lý do - **${reason || 'Không có lý do hoặc có thể là bạn đen hoặc do nhầm lẫn :D'}**`
          )
          .catch(() => null);
      } catch {
        message.guild.members.ban(banMember);
